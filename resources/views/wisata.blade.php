...
@section('main')
    ...
    <div class="modal fade text-left" id="tambahWisata" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33"
        aria-hidden="true">
        <div style="max-width:600px;" class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable"
            ...
            <div style="overflow-y: auto;" class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">Tambah Wisata</h4>
                ...
                <div class="modal-body">
                    <form action="" method="">
                        ...
                        <label class="m-0 mt-1">Harga Tiket Masuk</label>
                        <input placeholder="Tiket Masuk" class="form-control" id="tiket" type="number"
                        ...
                        <button type="button" id="btn-add-wisata" class="btn btn-primary mt-2">Tambah</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade text-left" id="editWisata" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33"
        aria-hidden="true">
        <div style="max-width:600px;" class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable"
            role="document">
            <div style="overflow-y: auto;" class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">Edit Wisata</h4>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i data-feather="x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="" method="">
                        ...
                        <label class="m-0">kategori</label>
                        <select class="form-control" id="kategoriedit" required>
                            <option value="">Pilih Kategori</option>
                            <option value="Buatan">Buatan</option>
                            <option value="Alam">Alam</option>
                        </select>
                        ...
                        <label class="m-0 mt-1">Harga Tiket Masuk</label>
                        <input placeholder="Tiket Masuk" class="form-control" id="tiketedit" type="number"
                            autocomplete="off" required>
                        <label class="m-0 mt-1">Deskripsi</label>
                        ...
                        <button type="button" id="btn-edit-wisata" class="btn btn-primary mt-2">Update</button>
                    </form>
                </div>
            </div>
        </div>
...
@push('script')
    <script type="text/javascript">
        ...
        function showModalEditWisata(id, nama, alamat, deskripsi, lat, lng, tiket_masuk, kategori, fasilitas) {
            if (markerTemp != "") {
                removeMarker(markerTemp, markerIdTemp)
                markerTemp = ""
                markerIdTemp = ""
            }
            $('#editWisata').modal('toggle');
            $("#fasilitasedit").html(tempChecks);
            $("#idedit").val(id);
            $("#namaedit").val(nama);
            $("#alamatedit").val(alamat);
            $("#deskripsiedit").val(deskripsi);
            $("#tiketedit").val(tiket_masuk);
        ...
        function inittable(data) {
            $('#listing-data').DataTable({
                ...
                buttons: [{
                        ...
                        title: 'Data Wisata ' + getDateToday(),
                        download: 'open',
                    ...
                    {
                        text: '<i class="fas fa-plus-circle"></i> Tambah Wisata',
                        "className": "generate-code",
                        action: function(e, dt, node, config) {
                            $('#tambahWisata').modal('toggle');
                            if (markerTemp != "") {
                                removeMarker(markerTemp, markerIdTemp)
                                markerTemp = ""
                                markerIdTemp = ""
        ...
        $('#btn-add-wisata').click(function(e) {
            ...
            if (kategori == "") {
                swal("error", "Kategori Tidak Boleh Kosong", "error")
            } else if (nama == "") {
                ...
                swal("error", "Tiket Tidak Boleh Kosong", "error")
            } else if (deskripsi == "") {
                ...
                myFormData.append('kategori', kategori);
        ...
        $('#btn-edit-wisata').click(function(e) {
            e.preventDefault();
            ...
            let tiket = $('#tiketedit').val().trim();
            let kategori = $('#kategoriedit option:selected').val();
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response) {
                                    swal("", "Hapus Wisata Berhasil", "success").then(function() {
                                        location.reload();
                                    });
                                }
                            },
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (let i = 0, len = data.length; i < len; i++) {
                        ...
                        let kategori = data[i].kategori;
                        let alamat = data[i].alamat;
                        let deskripsi = data[i].deskripsi;
                        let tiket = data[i].tiket_masuk;
                        ...
                        let btnAksi = "";
                        btnAksi =
                            `<a title='detail' href='/detailwisata/${id}' class='btn-aksi btn btn-sm btn-secondary'>Detail</a> 
                        <a title='accept' onclick="showModalEditWisata('${id}', '${nama}', '${alamat}', '${deskripsi}', '${lat}', '${lng}', '${tiket}', '${kategori}', '${fasilitas}')" class='btn-aksi btn btn-sm btn-success'>Edit</a>
                        <a title='delete' onclick="deleteWisata('${id}')" class='btn-aksi btn btn-sm btn-danger'>Hapus</a>
                        <a title='delete' onclick="showLocation('${nama}', '${lat}', '${lng}')" class='btn-aksi btn btn-sm btn-primary'>Map</a>
                        <a title='delete' href='/favorite/${id}?t=Wisata&n=${nama}' class='btn-aksi btn btn-sm btn-warning'>Favorite</a>
                        <a title='delete' href='/komentar/${id}?t=Wisata&n=${nama}' class='btn-aksi btn btn-sm btn-info'>Komentar</a>`;
                        arrayReturn.push([i + 1, nama, kategori, alamat, tiket, btnAksi]);
                    }
